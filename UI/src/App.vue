...
const fetchFiles = async () => {
  ...
  try {
    const res = await axios.get('http://localhost:8200/api/files', {
    ...
    if (res.data.files) {
      fileList.value = res.data.files.map((f: any) => ({
        ...f,
        // 使用新的下载 API，传入绝对路径
        url: `http://localhost:8200/api/download?path=${encodeURIComponent(f.path)}`
      }))
    }
  } catch (e) {
...
const connectWebSocket = () => {
  const ws = new WebSocket(`ws://localhost:8200/ws/${currentThreadId.value}`)
...
const handleSocketMessage = (data: any) => {
  ...
  if (event === 'session_created') {
    ...
    if (parts.length > 1) {
      currentSessionUrl.value = `http://localhost:8200/outputs/${parts[1].replace(/\\/g, '/')}`
    }
...
const sendMessage = async () => {
  ...
  if (selectedFiles.value.length > 0) {
    ...
    try {
        ...
        await axios.post('http://127.0.0.1:8200/api/upload', formData, {
            headers: {
  ...
  try {
    ...
    const res = await axios.post('http://127.0.0.1:8200/api/task', payload)
